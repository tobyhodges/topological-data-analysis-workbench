...
Given a set $ P=\\{p\_0,...,p\_k\\}\\subset \\mathbb{R}^d $ of $ k+1 $ affinely independent points, the **k-dimensional simplex** $\sigma$ (or **k-simplex** for short) spanned by $P$ is the set of convex combinatios

$ \\sum\_{i=0}^k\\lambda\_ip\_i, \\quad with \\quad  \\sum\_{i=0}^k\\lambda\_i = 1 \\quad \\lambda\_i \\geq 0. $
...
Given a point cloud $P=\\{p\_1,...,p\_n\\}\\subset \\mathbb{R}^d $, its **Rips complex** of radius $r>0$ is the simplicial complex $R(P,r)$ such that:  
$vert(R(P,r))=P$, and  
...
A filtration of a simplicial complex $K$ is a collection $K\_0\\subset K\_1 \\subset ...\\subset K\_N $ of complexes satisfying the two conditions:
